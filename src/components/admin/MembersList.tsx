  ...
  async function handleDelete(id: number) {
    ...
    try {
      ...
      if (res.ok) { // ok is a property of the Response object
        setData(prev => prev.filter(m => m.id !== id)); // Frontend state must be kept in sync with backend manually -> this line removes the deleted member from the UI immediately without waiting for a page refresh 
        setOpen(false);
        setSelectedId(null);
